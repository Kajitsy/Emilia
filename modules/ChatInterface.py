...
class GroupChatInterface(QWidget):
    def __init__(self, main_window, chat_data):
        super().__init__()
        self.mw = main_window
        self.chat_thread: ChatThread | None = self.mw.chat_thread
        self.discord_thread: DiscordRPC | None = self.mw.discord_thread
        ...
        self.chat_settings = QSettings(QSettings.Format.IniFormat, QSettings.Scope.UserScope, "Emilia", self.chat_id)
        self.char_back_message = self.chat_settings.value('colors/char_back_message', '#26272b')
        self.char_text_message = self.chat_settings.value('colors/char_text_message', '#e8eaed')
        self.user_back_message = self.chat_settings.value('colors/user_back_message', '#303136')
        self.user_text_message = self.chat_settings.value('colors/user_text_message', '#e8eaed')
    ...
    def createTopBar(self):
        ...
        self.header_chat_widget = QWidget()
        header_layout.addWidget(self.header_chat_widget)
        self.header_chat_widget.setVisible(False)
        header_character_layout = QHBoxLayout()
        self.header_chat_widget.setLayout(header_character_layout)
        ...
        header_char_text_layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header_character_layout.addLayout(header_char_text_layout)
        self.header_name_label = QLabel(self.chat_data['title'])
        header_char_text_layout.addWidget(self.header_name_label)
        self.header_author_label = QLabel()
        header_char_text_layout.addWidget(self.header_author_label)
    ...
    def createRightSidebar(self):
        self.chat_info_sidebar = QFrame(self)
        self.chat_info_sidebar.setFixedWidth(230)
        self.chat_info_sidebar.setFixedHeight(self.mw.height() - 230)
        self.chat_info_sidebar.setStyleSheet(character_info_sidebar_style())
        self.char_info_layout = QVBoxLayout()
        self.chat_info_sidebar.setLayout(self.char_info_layout)
        self.char_info_layout.setContentsMargins(10, 10, 10, 10)
        self.chat_info_sidebar.setVisible(False)
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if self.cis_visible:
            self.chat_info_sidebar.setGeometry(self.width() - self.chat_info_sidebar.width(), 0, 200, self.height() - 200)
        else:
            self.chat_info_sidebar.setGeometry(self.width(), 0, 200, self.height() - 200)
        self.chat_info_sidebar.setFixedHeight(self.mw.height() - 200)

    def showCharacterInfoSidebar(self):
        self.chat_info_sidebar.show()
        geom = self.chat_info_sidebar.geometry()
        self.animation = QPropertyAnimation(self.chat_info_sidebar, b"geometry")
        self.animation.setDuration(250)
        self.animation.setEasingCurve(QEasingCurve.Type.OutQuad)
        self.animation.setStartValue(geom)
        self.animation.setEndValue(QRect(geom.x() - self.chat_info_sidebar.width(), geom.y(), geom.width(), geom.height()))
        self.animation.start()
    ...
    def hideCharacterInfoSidebar(self):
        ...
        self.animation.setStartValue(geom)
        self.animation.setEndValue(QRect(geom.x() + self.chat_info_sidebar.width(), geom.y(), geom.width(), geom.height()))
        self.animation.finished.connect(self.chat_info_sidebar.hide)
        self.animation.start()
    ...
    def _getCharacter(self, character):
        ...
        self.title_label.setText(self.character.get('title'))
        self.toggle_info_button.setEnabled(True)
        self.header_chat_widget.setVisible(True)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if self.cis_visible:
            global_click_pos = event.globalPosition().toPoint()
            sidebar_global_pos = self.chat_info_sidebar.mapToGlobal(self.chat_info_sidebar.rect().topLeft())
            sidebar_rect = QRect(sidebar_global_pos, self.chat_info_sidebar.size())
            if not sidebar_rect.contains(global_click_pos):
                self.hideCharacterInfoSidebar2()
