...
class VoiceModeThread(QThread):
            ...
            if not self.muted:
                try:
                    ...
                    result = self.recognizer.recognize_google(
                        audio, language=self.lang.split("_")[0]
                    )
                    if not self.muted:
                    ...
                    logging.getLogger(__name__).warning(
                        f"QThreads.py ({self.__class__.__name__}.{inspect.currentframe().f_code.co_name}): Error converting speech to text"
                    )
            else:
                QThread.sleep(1)
    ...
    async def request(self, endpoint, data=None):
        ...
        response = await self.session.request(
            "POST",
            f"https://neo.character.ai/{endpoint}",
            headers=headers,
            json=data,
            timeout=100,
        )
        ...
        if response.status_code == 200:
            ...
            raise RuntimeError(f"Failed to get data, status code: {response.status_code}")

    async def tts(
        self, candidateId, roomId, turnId, voiceId: str = "", voiceQuery: str = ""
    ):
        if not voiceId:
            data = {
    ...
    async def _send_message(
        ...
        while True:
            ...
            if "turn" not in response:
                raise RuntimeError(response["comment"])
            yield response
    ...
    async def process_user_input(self):
        ...
        while True:
            ...
            audio_array, sample_rate = await self.tts(
                ai_message["primary_candidate_id"],
                self.chat_id,
                ai_message["turn_key"]["turn_id"],
                self.voice_id,
                ai_message["author"]["name"],
            )
