class ChatsAPI:
    ...
    async def get_chat(self, character_id: str):
        response = await self.client.request(f"chats/recent/{character_id}", domain="neo")
        return response.get("chats", [])

    async def get_chat_by_id(self, chat_id: str, load_metadata: bool = False):
        return await self.client.request(f"chat/{chat_id}/?load_metadata={load_metadata}", domain="neo")

    async def get_character_chats(self, character_id: str, num_preview_turns: int = 1):
        response = await self.client.request(
            f"chats/?character_ids={character_id}&num_preview_turns={num_preview_turns}", domain="neo")
        return response.get("chats", [])

    async def get_history(self, chat_id: str, next_token: str = None):
        url = f"turns/{chat_id}"
        if next_token:
        ...
        return {
            "turns": turns,
            "next_token": meta_token
        }

    async def remove_turns(self, chat_id: str, turn_ids: list):
        data = {"turn_ids": turn_ids}
        return await self.client.request(f"turns/{chat_id}/remove", data, "post", domain="neo")

    async def copy_chat(self, chat_id: str, end_turn_id: str):
        data = {"end_turn_id": end_turn_id}
        return await self.client.request(f"chat/{chat_id}/copy", data, "post", domain="neo")

    async def hide_chat(self, character_id: str):
        return await self.client.request(f"chats/recent/{character_id}/hide", method="put", domain="neo")
    ...
    async def get_recommended_chars(self):
        ...
        response = await self.client.custom_request("https://feed.api.character.ai/api/feed/recommended", method='post',
                                                    headers=headers)
    ...
    async def get_trythis_chats(self):
        ...
        res_data = []
        for i in response:
            res_data.append(i.get('result', {}).get("data", {}).get('json', {}).get('character'))
        return res_data
