...
class CharacterAPI:
    ...
    async def get_character(self, character_id=None, path=None):
        if path and not character_id:
            ...
            if match:
                ...
                url = f"character/{id_part}/{slug_part}.json?id={id_part}&slug={slug_part}" if slug_part else f"character/{id_part}.json?id={id_part}"
                ...
                pattern = re.compile(r'<script\s+id="__NEXT_DATA__"\s+type="application/json">\s*(\{.+?\})\s*</script>',
                                     re.DOTALL)
        ...
        voted = await self.client.request(f"character/v1/character_voted/{character_id}/voted", method="get",
                                          domain="neo")
    ...
    async def get_similar_characters(self, character_id):
        response = await self.client.request(f"recommendation/v1/character/similar/{character_id}", domain="neo")
    ...
    async def query_autocomplete(self, query_prefix: str):
        response = await self.client.request(f"search/v1/query/autocomplete?query_prefix={query_prefix}", method="get",
                                             domain="neo")
    ...
    async def vote_character(self, character_id: str, vote: bool | None):
        ...
        return await self.client.request("character/v1/vote_character", data, "post", domain="neo", text=True)

    async def get_upvoted_characters(self):
        response = await self.client.request("character/v1/upvoted_characters", method="get", domain="neo")
        return response.get('characters', [])

    async def create_character(self, data: dict):
        return await self.client.request("character/v1/create_character", data, "post", domain="neo")

    async def update_character(self, data: dict):
        return await self.client.request("character/v1/update_character", data, "post", domain="neo")

    async def get_user_personas(self, force_refresh=0):
        response = await self.client.request(f"character/v1/get_user_personas?force_refresh={force_refresh}",
                                             method="get", domain="neo")
        return response.get('personas', [])

    async def create_persona(self, avatar_rel_path: str, base_img_prompt: str, definition: str, name: str):
        data = {
            'avatar_file_name': "",
        ...
        response = await self.client.request("character/v1/create_persona", data, "post", domain="neo")
        return response.get('persona', {})

    async def update_persona(self, data: dict):
        response = await self.client.request("character/v1/update_persona", data, "post", text=True)
        return response.get('persona', {}) if isinstance(response, dict) else response

    async def remove_persona(self, data: dict):
        data['archived'] = True
        return await self.client.request("character/v1/update_persona", data, "post", text=True)

    async def get_category_characters(self, category: str):
        category_url = category.replace(' ', '%20').replace('&', '%26')
        response = await self.client.request(f"recommendation/v1/characters_with_tag/{category_url}", domain="neo")
