class UsersAPI:
    ...
    async def update_user_settings(self, data: dict):
        return await self.client.request("chat/user/update_settings/", data, "post", domain="neo")

    async def update_user_settings_2(self, data: dict):
        return await self.client.request("chat/user/update/", data, "post", domain="neo")
    ...
    async def get_user_following(self, page_param: int = 1, username: str = ""):
        ...
        return await self.client.request("chat/user/public/following/", data, "post", domain="plus")

    async def get_user_followers(self, page_param: int = 1, username: str = ""):
        data = {"pageParam": page_param, "username": username}
        return await self.client.request("chat/user/public/followers/", data, "post", domain="plus")
    ...
    async def upload_avatar(self, filetype: str, image_base64: str):
        data = {"0": {"json": {"imageDataUrl": f"data:image/{filetype};base64,{image_base64}"}}}
        response = await self.client.request("user.uploadAvatar?batch=1", data, "post", domain="trpc")
        return response[0].get('result', {}).get('data', {}).get('json', {})
