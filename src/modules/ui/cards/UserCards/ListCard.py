...
class ListCard(CardFrame):
        ...
        if self.data.get("account__avatar_file_name"):
            self.mw.image_loader.load(
                ...
                error_cb=lambda _: color_avatar(
                    self.image_label, 70, 70, self.username, 4
                ),
            )
        else:
        ...
        if self.data.get("character_info", {}):
            ...
            if self.data.get("character_info", {}).get("num_interactions"):
                self.interactions_label = QLabel(
                    format_number(
                        self.data.get("character_info", {}).get("num_interactions")
                    )
                    + self.tr(" Chats")
                )
            ...
            if self.data.get("character_info", {}).get(
                "num_interactions"
            ) and self.data.get("character_info", {}).get("num_characters"):
            ...
            if self.data.get("character_info", {}).get("num_characters"):
                self.characters_label = QLabel(
                    format_number(
                        self.data.get("character_info", {}).get("num_characters")
                    )
                    + self.tr(" Characters")
                )
    ...
    def update_theme(self):
        ...
        if self.data.get("character_info", {}):
            if self.data.get("character_info", {}).get("num_interactions"):
                self.interactions_label.setStyleSheet(
                    f"color: {TM.c('disabled_text')};"
                )
