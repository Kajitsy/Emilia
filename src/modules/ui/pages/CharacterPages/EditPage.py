...
class EditPage(QWidget):
    ...
    def initUI(self):
        ...
        scroll_layout.addWidget(
            self.character_name_label, alignment=Qt.AlignmentFlag.AlignLeft
        )
        ...
        scroll_layout.addWidget(
            self.tagline_label, alignment=Qt.AlignmentFlag.AlignLeft
        )

        self.tagline_edit = LineEdit()
        self.tagline_edit.setMaxLength(50)
        self.tagline_edit.setPlaceholderText(
            self.tr("Add a short tagline of your Character")
        )
        ...
        description_layout.addWidget(
            self.description_label, alignment=Qt.AlignmentFlag.AlignLeft
        )
        ...
        self.description_edit.setPlaceholderText(
            self.tr("How would your Character describe themselves?")
        )
        self.description_edit.textChanged.connect(
            lambda: self.textChanged(self.description_edit, 500)
        )
        ...
        self.greeting_edit.setPlaceholderText(
            self.tr(
                "e.g. Hello, I am Albert. Ask me anything about my scientific contributions."
            )
        )
        self.greeting_edit.textChanged.connect(
            lambda: self.textChanged(self.greeting_edit, 4096)
        )
        ...
        definition_layout.addWidget(
            self.definition_label, alignment=Qt.AlignmentFlag.AlignLeft
        )
        ...
        heading_combo.addItem(self.tr("###### Title 6"), "######")
        heading_combo.currentIndexChanged.connect(
            lambda: self.definition_edit.applyHeading(heading_combo)
        )
        definition_layout.addWidget(heading_combo)
        bold_button = PushButton(self.tr("Bold"))
        bold_button.clicked.connect(
            lambda: self.definition_edit.formatSelectedText("**", "**")
        )
        ...
        self.definition_edit.setPlaceholderText(
            self.tr(
                "What's your Character's backstory? How do you want it to talk or act?"
            )
        )
        self.definition_edit.textChanged.connect(
            lambda: self.textChanged(self.definition_edit, 32000)
        )
    ...
    def _saveCharacterChat(self, data):
        if data.get("status") == "OK":
            self.mw.showNotification(
                self.tr("The character has been saved successfully!")
            )
    ...
    def _createCharacter(self, data):
        if data.get("status") == "OK":
            self.mw.showNotification(
                self.tr("The character has been successfully created!")
            )
            char = data.get("character")
            self.mw.openChat(char["external_id"], char["name"])
        else:
    ...
    def saveData(self):
        if len(self.character_name_edit.text()) <= 2:
            self.mw.showNotification(
                self.tr("The character's name must be longer than three characters.")
            )
            return
        if len(self.greeting_edit.toPlainText()) <= 2:
            self.mw.showNotification(
                self.tr(
                    "The character's greeting must be longer than three characters."
                )
            )
            return
    ...
    def loadData(self, data):
        ...
        if self.data and self.has_data:
            ...
            if self.data.get("avatar_rel_path"):
                self.image_loader.load(
                    f"https://characterai.io/i/80/static/avatars/{self.data['avatar_rel_path']}?webp=true&anim=0",
                    60,
                    60,
                    100,
                    label=self.display_avatar,
                    error_cb=lambda _: color_avatar(
                        self.display_avatar, 60, 60, data["name"]
                    ),
                )
            else:
            ...
            self.dg_checkbox.setChecked(
                self.data.get("dynamic_greeting_enabled", False)
            )
